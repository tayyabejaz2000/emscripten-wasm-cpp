...
Vector& Vector::Mul(const Vector& v)
{
    this->vec *= v.vec;
    return *this;
}

Vector& Vector::Div(const Vector& v)
{
    this->vec /= v.vec;
    return *this;
}

Vector& Vector::AddVal(float val)
{
    this->vec += val;
    return *this;
}

Vector& Vector::SubVal(float val)
{
    this->vec -= val;
    return *this;
}

Vector& Vector::MulVal(float val)
{
    this->vec *= val;
    return *this;
}

Vector& Vector::DivVal(float val)
{
    this->vec /= val;
    return *this;
}
...
{
    return Vector(this->vec / this->Magnitude());
}
...
Vector Vector::Midpoint(const Vector a, const Vector b)
{
    return Vector((a.vec + b.vec) / 2);
}